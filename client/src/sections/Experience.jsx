          ...
          {Experience.map((exp, idx) => (
            <div key={idx} className={`w-[20px] h-[20px] rounded-full relative bg-black`}>
              ...
              <motion.div
                initial={{ 
                  opacity: 0, 
                  x: idx % 2 === 0 ? -80 : 80   // even → left, odd → right
                }}
              ...
              >
                ...
                <h1 className='flex items-center border mt-1 justify-between px-1 rounded-lg'>
                  <div className='flex items-center justify-center gap-1'>
                    <span className='text-xs'>{exp.joiningDate}</span>-
                    {exp.currentlyWorking && exp.lastDate === "" ? (
                      ...
                      <span className='text-xs'>{exp.lastDate}</span>
                    )}
                  </div>
              ...
              >
                ...
                {exp.logo ? (
                  <img
                    src={exp.logo}
                    alt={exp.company}
                    className={`w-20 absolute -top-4 ${
                      idx % 2 === 0 ? "-right-3" : "-left-3"
                    }`}
                  />
                ) : (
                  <span className='w-[50px] h-[50px] border rounded-full absolute -top-1 left-[2px] flex items-center justify-center text-2xl'>
